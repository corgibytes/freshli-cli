...
public class ComputeHistoryCommandRunner : CommandRunner<ComputeHistoryCommand, ComputeHistoryCommandOptions>
{
    ...
    public override int Run(ComputeHistoryCommandOptions options, InvocationContext context)
    {
        ...
        {
            WriteStopsToLines(
                _computeHistory.ComputeCommitHistory(new AnalysisLocation(options.RepositoryId, options.CacheDir),
                    options.GitPath),
                context);
            return 0;
        }
        ...
        WriteStopsToLines(
            _computeHistory
                .ComputeWithHistoryInterval(new AnalysisLocation(options.RepositoryId, options.CacheDir),
