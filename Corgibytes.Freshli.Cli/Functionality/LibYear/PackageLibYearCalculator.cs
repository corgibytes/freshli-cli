...
{
    public PackageLibYear ComputeLibYear(IAgentReader agentReader, PackageURL packageUrl, DateTimeOffset asOfDateTime, ICacheManager cacheManager)
    {
        try
        {
            var releaseHistory = agentReader.RetrieveReleaseHistory(packageUrl, cacheManager);
    ...
    private static DateTimeOffset GetReleaseDate(IEnumerable<CachedPackage> releaseHistory, PackageURL packageUrl)
    {
    ...
    private static PackageURL GetLatestVersion(IEnumerable<CachedPackage> releaseHistory, DateTimeOffset asOfDate)
    {
