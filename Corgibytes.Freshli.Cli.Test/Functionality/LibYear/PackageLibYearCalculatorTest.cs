...
public class PackageLibYearCalculatorTest
{
    ...
    public void VerifyItCanCalculateTheLibYear()
    {
        ...
        var givenReleaseHistory = new List<CachedPackage>
        {
            new() // This version was released _after_ the asOfDateTime so it should not count
            {
                PackageUrl = new PackageURL(packageName + "@2.3.1"),
                ReleasedAt = new DateTimeOffset(2022, 1, 29, 12, 30, 45, TimeSpan.Zero)
            },
            new()
            {
                PackageUrl = new PackageURL(packageName + "@2.2.1"),
                ReleasedAt = new DateTimeOffset(2021, 1, 29, 12, 30, 45, TimeSpan.Zero)
            },
            new()
            {
                PackageUrl = new PackageURL(packageName + "@2.0.0"),
                ReleasedAt = new DateTimeOffset(2020, 2, 29, 12, 30, 45, TimeSpan.Zero)
            },
            new()
            {
                PackageUrl = new PackageURL(packageName + "@1.3.5"),
                ReleasedAt = new DateTimeOffset(2019, 6, 1, 12, 30, 45, TimeSpan.Zero)
            },
            new()
            {
                PackageUrl = currentlyInstalledPackageUrl,
                ReleasedAt = new DateTimeOffset(2019, 3, 30, 14, 41, 55, TimeSpan.Zero)
            },
        };

        var agentReader = new Mock<IAgentReader>();
        var cacheManager = new Mock<ICacheManager>();
        agentReader.Setup(mock => mock.RetrieveReleaseHistory(currentlyInstalledPackageUrl, cacheManager.Object)).Returns(givenReleaseHistory);
        ...
        var actualPackageLibYear = calculator.ComputeLibYear(agentReader.Object, currentlyInstalledPackageUrl, asOfDateTime, cacheManager.Object);
